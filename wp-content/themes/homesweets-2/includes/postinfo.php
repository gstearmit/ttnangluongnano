...
<?php } elseif (is_single() && get_option('chameleon_postinfo2') <> '') { ?>
	<p class="description">
		...
		while ($new_query->have_posts()) $new_query->the_post(); ?>
			<?php esc_html_e('','Chameleon'); ?> 
			<?php if (in_array('author', get_option('chameleon_postinfo2')))
			   { ?> <?php esc_html_e('by','Chameleon'); ?> <?php the_author_posts_link(); ?><?php 
			   }; ?>
			   <?php if (in_array('date', get_option('chameleon_postinfo2'))) 
			      { ?> <?php esc_html_e('on','Chameleon'); ?> <?php the_time(esc_attr(get_option('chameleon_date_format'))) ?>
			<?php }; ?>
			   <?php if (in_array('categories', get_option('chameleon_postinfo2'))) 
			     { ?> <?php esc_html_e('in','Chameleon'); ?> <?php the_category(', ') ?><?php }; ?>
