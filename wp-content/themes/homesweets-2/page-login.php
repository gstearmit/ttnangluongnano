...
<?php
	function checkEmailExists($email){
		$exists = 0;
		$url_domain_email = 'http://ezsolution.vn:8180/EzRest/rest/web/getAccountByEmail?e='.$email;
	...
	function checkContactPhoneExists($contactPhone){
		$exists = 0;
		$url_domain_phone = 'http://ezsolution.vn:8180/EzRest/rest/web/getAccountByUserName?u='.$contactPhone;
	...
	function registerAccount(){
		...
		$ch = curl_init('http://ezsolution.vn:8180/EzRest/rest/web/registerAccount');
		...
		$et_register_email_message = '<html>
			...
			<div class="ex">
				...
				<p style="margin-left:5px;">
					<label style="color:#5C5C5C">Hỗ trợ kỹ thuật: 0978276705  &nbsp; |  &nbsp;  Hỗ trợ kinh doanh: 0932255068  &nbsp;  |   &nbsp; hoặc gửi email cho chúng tôi: hotro@maybanhang.net </label>
				</p>
				<label style="color:#5C5C5C">
...
<?php
	...
	if ( isset($_POST['et_register_form_submit']) || isset($_POST['et_contact_reset']) || isset($_POST['register_submit_form'])) {
		...
		if ( !isset($_POST['et_register_form_email']) || empty($_POST['et_register_form_email']) ) {
			...
			if($emailExists == 1){
				...
				$et_error_message .= '<p>' . esc_html__('Lỗi kết nối với hệ thống. ','Chameleon') . '</p>';
				$et_register_form_error = true;
			}
		}
	   ...
	   if (!$resp->is_valid) {
		   // xu li code cu minh o day
		   // echo "Bạn Đã Nhập Đúng!";
	   } else {
		   # set the error code so that we can display it
		   // $error = $resp->error;
		   $et_error_message .= '<p>' . esc_html__('Bạn nhập sai Mã bảo vệ. ','Chameleon') . '</p>';
           $et_register_form_error = true;
	   }
	} else {
		$et_register_form_error = true;
	...
	if ( !$et_register_form_error ) {
		...
		header('Location: http://'.$baseUrl.'/29227736bbfdcd42d2d4c7447b9df8fa.php?accountNo='.md5($accountNo));

		//$et_error_message = '<p>' . esc_html__('Bạn đã đăng ký tài khoản thành công!','Chameleon') . '</p>';
		//$et_error_message .= '<p>Bấm vào <a href="http://quanly.maybanhang.net" target="_blank">đây để đăng nhập hệ thống.</a></p>';
	}
?>
	...
	<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
		<div class="entry post clearfix">
			...
			<div id="et-register">
				...
				<div id="et-contact-message" style="color:#FF0000">
					<?php echo($et_error_message); ?>
			    </div>

				<?php if ( $et_register_form_error ) { ?>

				<div id='dangki_vistor' style='width:583px;height: 800px; overflow:'>

					<form action="<?php echo(get_permalink($post->ID)); ?>" method="post" id="et_register_form">
						<div id='namedangki_form' style='width:583px;color: black;font-size: 15px;padding: 10px;height: 20px;'>
							<p class="dki_use">ĐĂNG KÝ SỬ DỤNG </p><h5 id='fontchu_dep'>(7 ngày dùng thử miễn phí tất cả các gói)</h5>
						</div>

						<ul id="et_contact_left">

							<li id='li_ul'>
							 	<div id='fontName'> Nhu cầu đăng ký </div>
							 	<div id='left_div' style='float: right;'>
									<select class='opptincee' id="et_register_form_account_type"  name="et_register_form_account_type">
										<option value="">-----</option>
									...
									<div id='atrr_true'>*</div>
								</div> <!-- id='left_div'-->
							</li>


							<li id='li_ul'>
								<div id='fontName'> Gói dịch vụ </div>
								<div id='left_div' style='float: right;'>
									<select class='opptincee' id="et_register_form_service"  name="et_register_form_service">
							...
							<li id='li_ul'>
							    <div id='fontName' >Họ tên </div>
							    <div id='left_div' style='float: right;'>
							    	<input class='input_text' type="text" name="et_register_form_full_name" value="<?php if ( isset($_POST['et_register_form_full_name']) ) echo esc_attr($_POST['et_register_form_full_name']); ?>" id="et_register_form_full_name"  />
									<div id='atrr_true'>*</div>
								</div> <!-- id='left_div'-->
						    </li>

						    <li id='li_ul'>
							    <div id='fontName' >Tên công ty </div>
							     <input class='input_text' type="text" name="et_register_form_company_name" value="<?php if ( isset($_POST['et_register_form_company_name']) ) echo esc_attr($_POST['et_register_form_company_name']); ?>" id="et_register_form_company_name"  />
						    </li>

						    <li id='li_ul'>
								<div id='fontName'> Ngành nghề kinh doanh</div>
								<div id='left_div' style='float: right;'>
									<select class='opptincee' id="et_register_form_industry"  name="et_register_form_industry">
							...
							<li id='li_ul'>
							    <div id='fontName' >Tên cửa hàng </div>
							    <div id='left_div' style='float: right;'>
							    	<input class='input_text' type="text" name="et_register_form_location_name" value="<?php if ( isset($_POST['et_register_form_location_name']) ) echo esc_attr($_POST['et_register_form_location_name']); ?>" id="et_register_form_location_name"  />
							    	<div id='atrr_true'>*</div>
								</div> <!-- id='left_div'-->
						    </li>

						    <li id='li_ul'>
								<div id='fontName'>Tỉnh</div>
								<div id='left_div' style='float: right;'>
									<select class='opptincee' id="et_register_form_state"  name="et_register_form_state">
							...
							<li id='li_ul'>
							    <div id='fontName' >Email </div>
							    <div id='left_div' style='float: right;'>
							   		<input class='input_text' type="text" name="et_register_form_email" value="<?php if ( isset($_POST['et_register_form_email']) ) echo esc_attr($_POST['et_register_form_email']); ?>" id="et_register_form_email"  />
							    	<div id='atrr_true'>*</div>
								</div> <!-- id='left_div'-->
						    </li>

						    <li id='li_ul'>
							    <div id='fontName' >Điện thoại di động </div>
							    <div id='left_div' style='float: right;'>
							   		<input class='input_text' type="text" name="et_register_form_contact_phone" value="<?php if ( isset($_POST['et_register_form_contact_phone']) ) echo esc_attr($_POST['et_register_form_contact_phone']); ?>" id="et_register_form_contact_phone"  />
							    	<div id='atrr_true'>*</div>
								</div> <!-- id='left_div'-->
						    </li>
						     <li id='li_ul'>
						        <div  id='dt_dt' >(Dùng làm tên đăng nhập)</div>
						     </li>

						    <li id='li_ul'>
							    <div id='fontName_nn' >Mật khẩu </div>
							     <div id='left_div' style='float: right;'>
							    	<input class='input_text' type="password" name="et_register_form_password" value="<?php if ( isset($_POST['et_register_form_password']) ) echo esc_attr($_POST['et_register_form_password']); ?>" id="et_register_form_password"  />
							    	<div id='atrr_truenn'>*</div>
								</div> <!-- id='left_div'-->
						    </li>

						    <li id='li_ul'>
							    <div id='fontName_nn' >Xác nhận mật khẩu </div>
							     <div id='left_div' style='float: right;'>
							   		<input class='input_text' type="password" name="et_register_form_password_confirm" value="<?php if ( isset($_POST['et_register_form_password_confirm']) ) echo esc_attr($_POST['et_register_form_password_confirm']); ?>" id="et_register_form_password_confirm"  />
							     	<div id='atrr_truenn'>*</div>
								</div> <!-- id='left_div'-->
						    </li>

						    <li id='li_ul'>
							    <div id='fontName_nn' >Mã bảo vệ </div>
							   <div id='catcha_p' style='float: right;position: relative;top: -21px;left: 28px;'>
						<?php
							...
							//var_dump(recaptcha_get_html($publickey));
						...
						<input  type="hidden" name="et_register_form_submit" value="et_register_form_proccess" />
						<input  type="reset" id="et_contact_reset" value="<?php esc_attr_e('Nhập lại','Chameleon');?>"/>
						<input type="hidden" name="base_url" id="base_url" value="<?php echo $baseUrl; ?>" />
						<input type="hidden" name="urll" id="urll" value="<?php echo $urll; ?>" />
						<input  type="submit" value="<?php esc_attr_e('Đăng ký','Chameleon'); ?>" id="register_submit_form" />
					</form>
